...
| ------------- | -----: | ----------------- | ------------- | --------------------------- |
| Control-plane | `8889` | UDP broadcast     | `bucis → brs` | `sound_start`, `sound_stop` |
| Media-plane   | `5006` | RTP/UDP broadcast | `bucis → brs` | IMA ADPCM payload (PT=96 по умолчанию; динамический) |
...
- **Payload type**: `96` (динамический; можно изменить переменной `RTP_IMAADPCM_PT`)

В этой симуляции используется IMA ADPCM как нестандартный плейсхолдер (не ITU-T G.726). Кодек **не совместим** с реализациями G.726 и типичными RTP/GStreamer пайплайнами для G.726 (`avenc_g726`, `rtpg726pay`, `avdec_g726`).
...
- **`RTP_IMAADPCM_PT`**: RTP payload type для IMA ADPCM, default `96` (динамический диапазон `96-127`)
