...
Проект имитирует работу аудиоподсистемы «Сармат» с помощью UDP broadcast.
...
- **BRS** — получатель. По команде `sound_start` планирует старт медиаприёмника на момент \(t0\). Ровно в \(t0\) открывает UDP-сокет для media-plane и принимает RTP-пакеты, декодирует G.726 в PCM (PCM отбрасывается — ALSA недоступна), собирает метрики: число принятых пакетов, потери, jitter.
...
- **`CONTROL_ADDR`**: broadcast-адрес подсети (например `10.10.10.255`)
  - для `bucis`: **обязателен**
  - для `brs`: если не задан, используется **`10.10.10.255`** (автоопределения broadcast-адреса подсети сейчас нет)
